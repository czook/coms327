...
int test_loc(dungeon_t * dungeon, int x, int y, character_t *c) {
	if(x > 0 && x < DUNGEON_X-1 && y > 0 && y < DUNGEON_Y-1) {
		int hard = dungeon->hardness[y][x];
		if(dungeon->hardness[y][x] < 255) {
			if(c->c.npc.tunneling == 0 && hard > 0)
				return 0;
			return 1;
		}
	}
...
int with_pc(dungeon_t * dungeon, character_t * s) {
	int pc_rid	= -1;
	int s_rid	= -1;
	pair_t *pc = &(dungeon->pc);

	int i;
	for(i = 0; i < dungeon->nr; i++) {
		if(s->pos[0] <= dungeon->rooms[i].position[0] + dungeon->rooms[i].size[0] && s->pos[1] <= dungeon->rooms[i].position[1] + dungeon->rooms[i].size[1] && s->pos[0] >= dungeon->rooms[i].position[0] && s->pos[1] >= dungeon->rooms[i].position[1]) {
			s_rid = i;
		}
		if(pc[0] <= dungeon->rooms[i].position[0] + dungeon->rooms[i].size[0] && pc[1] <= dungeon->rooms[i].position[1] + dungeon->rooms[i].size[1] && pc[0] >= dungeon->rooms[i].position[0] && pc[1] >= dungeon->rooms[i].position[1]) {
			pc_rid = i;
		}
	}
...
void gen_move_sprite(dungeon_t * dungeon, int sn) {
...
void add_sprite(dungeon_t * dungeon, Sprite s) {
...
*/
Sprite gen_sprite(dungeon_t * dungeon, char c, int x, int y, int r) {
	Sprite s;
...
bool check_any_monsters(dungeon_t * dungeon) {
	int i;
	for(i = 0; i < dungeon->ns; i++) {
